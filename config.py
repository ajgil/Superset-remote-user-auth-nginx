...
# -----------------------------------------------------
# Comentar o descomentar para iniciar en un dominio u otro
# AUTH_TYPE = AUTH_LDAP
...
# CONFIG ASEPEYO LDAP
# ----------------------------------------------------
# Comentar o descomentar para inciar en un dominio o en otro
#AUTH_TYPE = AUTH_LDAP
...
# ----------------------------------------------------------------
# Login automático mediante paso de parámetros a Superset DB
# ----------------------------------------------------------------
'''
class CustomAuthDBView(AuthDBView):
    ...
    def login(self):
        # Paso por headers NO FUNCIONA
#        if request.headers.get('username') is not None:
#            user = self.appbuilder.sm.find_user(username=request.headers.get('username'))
#            print('headers: ', user)
#            flash('Usuario auto logged in', 'success')
...
#            return redirect(self.appbuilder.get_url_for_index)
        ...
        if request.args.get('username') is not None:
            ...
            print('No se ha recibido header')
            flash('No se ha podido realizar el auto login', 'warning')
...
# ----------------------------------------------------------------
# Login mediante AUTH_REMOTE_USER
# ----------------------------------------------------------------
# 1.- Definir una clase middleware que capture la variable y la pase al entorno de Gunicorn
# 2.- Actualizar la variable global ADDITIONAL_MIDDLEWARE
# 3.- override vista auth remota con login
...
class AsepeyoRemoteUserView(AuthRemoteUserView):
    # Probamos a usar la plantilla de login ldap
    login_template = ''

    @expose('/login/')
    def login(self):
        # headers
        username = request.headers.get('HTTP_X_PROXY_REMOTE_USER')
        # username = request.headers.get('REMOTE_USER')
        print ('Username: ', username)

        if g.user is not None and g.user.is_authenticated():
            print('Usuario global')
            return redirect(self.appbuilder.get_url_for_index)
        ...
        if user is None and username:
            ...
            msg = ("Usuario no permitido, {}".format(username))
        ...
        if username:
            ...
            flash(as_unicode(self.invalid_login_message), 'warning')
            print('Estamos en un warning')

        return redirect(self.appbuilder.get_url_for_index)

class AsepeyoSecurityManager(SupersetSecurityManager):
    authremoteuserview = AsepeyoRemoteUserView
...
# Configurando workers Celery y broker Redis
